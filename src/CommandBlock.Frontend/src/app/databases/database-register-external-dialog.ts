  ...
  template: `
    <hlm-dialog-header>
      ...
      <p hlmDialogDescription>
        Connect to a database CommandBlock didn't provision. We store the credentials so you can browse,
        query, and manage users from the UI. CommandBlock will <strong>not</strong> own its lifecycle:
        upgrades, backups, and container controls are disabled for external instances.
      </p>
    </hlm-dialog-header>

    <!-- Discover panel. Scans the host's Docker for engine containers CommandBlock doesn't already
          ...
          @for (c of candidates(); track c.containerId) {
            <li class="flex items-center justify-between gap-3 px-3 py-2">
              ...
              <div class="flex items-center gap-1.5">
                @if (canMigrate(c)) {
                  <button hlmBtn size="sm" variant="outline" type="button" (click)="migrate(c)" title="Provision a fresh CommandBlock instance and copy data into it">
...
export class DatabaseRegisterExternalDialog {
  ...
  // "adopted Docker container" - CommandBlock enables upgrade/backup/exec against it even though
