...
@Component({
  ...
  template: `
    <hlm-dialog-header>
      <h3 hlmDialogTitle>Migrate into CommandBlock</h3>
      <p hlmDialogDescription>
        Provision a fresh CommandBlock-managed instance and copy data from <strong>{{ candidate.containerName }}</strong> into it.
    ...
    @switch (phase()) {
      @case ('review') {
        ...
        <p class="text-muted-foreground text-xs">
          Tip: stop writes to the source database before starting. CommandBlock does not lock it, so concurrent writes during the dump may be lost.
        </p>
...
export class DatabaseMigrateDialog {
  ...
  private defaultTargetName(c: DiscoveredContainerDto): string {
    return c.composeService ? `${c.composeService}-commandblock` : `${c.containerName}-commandblock`;
  }
}
