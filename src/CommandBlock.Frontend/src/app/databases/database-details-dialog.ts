    ...
    @if (store.loadingDetails()) {
      ...
      @if (!d.isManaged) {
        <div class="border-amber-500/30 bg-amber-500/5 rounded-md border p-3 text-sm">
          <p class="font-medium">This database is not managed by CommandBlock.</p>
          <p class="text-muted-foreground mt-1">
            @if (d.containerName) {
              It was adopted from an existing Docker container. CommandBlock can back up, exec, and
              ...
              controls are disabled — CommandBlock only handles browsing, querying, and user management
