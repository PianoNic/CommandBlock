namespace CommandBlock.Application.Dtos.DatabaseInstance
{
    ...
    /// CommandBlock will store the credentials, surface the DB in the UI for browsing/querying/user
    ...
    /// is a Docker container CommandBlock can reach via the local daemon, typically discovered through
    /// the Scan flow), CommandBlock enables the full operational feature set against it - upgrades,
    /// backups, container logs, exec - because those operations only need a reachable container.
    /// The instance is still flagged IsManaged=false so deletion stays a "forget" (CommandBlock will
