...
namespace CommandBlock.Application
{
    ...
    {
        public static async Task<InnerDatabaseTarget> LoadAsync(CommandBlockDbContext db, ISecretsVaultService vault, Guid instanceId, CancellationToken cancellationToken)
        {
        ...
        // Prefers reaching a CommandBlock-provisioned container directly over the shared Docker network
        // (containerName:internalPort) - which works for private instances a containerized CommandBlock
        // can't otherwise reach - and falls back to the host-published port when that's unavailable
        // (host-run CommandBlock, or the container isn't on our network). Cached per endpoint.
        ...
        // directly over CommandBlock's Docker network. Mirrors the engine catalog; 0 = no direct route.
        ...
        // probe cost; on a host-run CommandBlock (e.g. the desktop app) the first-choice host can be
        ...
        // Which local probe host can reach a published port depends on where CommandBlock runs: a
        // containerized CommandBlock can't reach 127.0.0.1-bound ports via its own loopback but can
        // via Docker Desktop's host-gateway (host.docker.internal); a host-run CommandBlock is the
        ...
        // the commandblock container that's commandblock's own loopback, not the docker host, so translate:
