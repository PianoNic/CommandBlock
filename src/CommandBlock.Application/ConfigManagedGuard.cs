...
namespace CommandBlock.Application
{
    ...
    public sealed class ConfigManagedGuard
    {
        ...
        public void EnsureMutable(DatabaseInstance instance)
        {
            if (instance.IsConfigManaged && !Bypass)
            {
                throw new InvalidOperationException(
                    $"'{instance.DisplayName}' is owned by instances.yaml. Remove the entry from the config and restart CommandBlock to manage it via the UI.");
            }
        }
        ...
        public async Task EnsureMutableAsync(CommandBlockDbContext db, Guid instanceId, CancellationToken cancellationToken)
        {
            if (Bypass) return;
            ...
            {
                throw new InvalidOperationException(
                    $"'{flag.DisplayName}' is owned by instances.yaml. Remove the entry from the config and restart CommandBlock to manage it via the UI.");
            }
        }
    }
}
