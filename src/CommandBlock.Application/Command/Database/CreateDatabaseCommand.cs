...
using CommandBlock.Application.Mappings.Database;
...
namespace CommandBlock.Application.Command.Database
{
    ...
    public class CreateDatabaseCommandHandler(IDockerServiceResolver dockerResolver, ISecretGeneratorService secretGenerator, ISecretsVaultService vault, CommandBlockDbContext db, IOptions<CommandBlockOptions> options, IActivityLogger activity, IInnerDatabaseServiceResolver innerDbs) : ICommandHandler<CreateDatabaseCommand, ProvisionedDatabaseDto>
    {
        private const string Host = "localhost";
        // commandblock runs in its own container; localhost there is commandblock's loopback, not the Docker
        ...
        /// is published on 0.0.0.0 (IsPublic=true) use host.docker.internal so CommandBlock-in-a-container
        ...
        public static InnerDatabaseTarget BuildProbeTarget(CommandBlock.Domain.DatabaseInstance instance, string password)
        {
        ...
        public async ValueTask<ProvisionedDatabaseDto> Handle(CreateDatabaseCommand command, CancellationToken cancellationToken)
        {
            ...
            var containerName = $"commandblock-{spec.ShortName}-{instanceIdShort}";
            ...
            {
                CommandBlock.Infrastructure.Services.SafePasswordGuard.Require(command.Password);
            ...
            var createParams = new CreateContainerParameters
            {
                ...
                Labels = Containers.CommandBlockContainerLabels.For(command.Engine, instanceId, command.DisplayName),
            };
            ...
            try
            {
                ...
                var instance = new CommandBlock.Domain.DatabaseInstance
                {
                    Id = instanceId,
        ...
        internal static EngineSpec ResolveEngineSpec(string engine, string version)
        {
            switch (engine)
            {
                ...
                case "seaweedfs":
                    ...
                    return new EngineSpec("chrislusf/seaweedfs", "weed", 8333, "commandblock", "default", "/data",
        ...
        private static string ResolveDatabaseName(EngineSpec spec, string? requested)
        {
            ...
            CommandBlock.Infrastructure.Services.InnerDatabaseNameValidator.Require(requested);
            return requested;
        }
        ...
        private async Task<int> AllocateHostPortAsync(string engine, Guid? nodeId, IDockerService docker, CancellationToken cancellationToken)
        {
            ...
            // 1) Ports CommandBlock already recorded as in-use for this engine on the same target (port
            ...
            //    left over from a wiped commandblock DB, prior deploys, or anything else binding a port
