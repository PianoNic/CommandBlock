...
using CommandBlock.Application.Mappings.DatabaseInstance;
...
namespace CommandBlock.Application.Command.Database
{
        ...
        : ICommandHandler<UpgradeDatabaseCommand, DatabaseInstanceDto>
    {
        public async ValueTask<DatabaseInstanceDto> Handle(UpgradeDatabaseCommand command, CancellationToken cancellationToken)
        {
            ...
            // the next `docker compose up` would recreate the old container next to CommandBlock's new
            // one and break the connection. Only CommandBlock-provisioned instances may upgrade.
            if (!instance.IsManaged || instance.ContainerName is null || instance.ContainerId is null)
                throw new InvalidOperationException("Upgrade is only available for CommandBlock-managed databases. External containers are owned by the orchestrator that created them (e.g. docker compose) - upgrade them there.");
            ...
            var newContainerName = $"commandblock-{spec.ShortName}-{newInstanceShort}";
            ...
            var createParams = new CreateContainerParameters
            {
                ...
                Labels = Containers.CommandBlockContainerLabels.For(instance.Engine, instance.Id, instance.DisplayName),
            };
