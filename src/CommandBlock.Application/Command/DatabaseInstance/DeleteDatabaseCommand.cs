...
namespace CommandBlock.Application.Command.DatabaseInstance
{
    ...
    public class DeleteDatabaseCommandHandler(CommandBlockDbContext db, IDockerServiceResolver dockerResolver, ISecretsVaultService vault, IActivityLogger activity, IOptions<CommandBlockOptions> options, ConfigManagedGuard guard) : ICommandHandler<DeleteDatabaseCommand>
    {
        public async ValueTask<Unit> Handle(DeleteDatabaseCommand command, CancellationToken cancellationToken)
        {
            ...
            // External instances are "forgotten" - CommandBlock didn't create the database, so it doesn't
