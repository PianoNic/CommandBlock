...
namespace CommandBlock.Application.Command.DatabaseInstance
{
    ...
    /// <summary>
    /// Registers an existing, externally-hosted database with CommandBlock. No container is created -
    /// CommandBlock simply stores the credentials + engine metadata so the UI can browse/query/manage
        ...
        : ICommandHandler<RegisterExternalDatabaseCommand, ProvisionedDatabaseDto>
    {
        public async ValueTask<ProvisionedDatabaseDto> Handle(RegisterExternalDatabaseCommand command, CancellationToken cancellationToken)
        {
            ...
            // localhost/127.0.0.1 means different things depending on where CommandBlock runs: its own
