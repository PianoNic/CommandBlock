...
namespace CommandBlock.Application.Command.DatabaseInstance
{
    ...
    /// </list>
    /// Externals (no containerId) and adopted-Docker externals are rejected: CommandBlock doesn't
        ...
        : ICommandHandler<SetInstanceRootPasswordCommand, InnerUserPasswordDto>
    {
        public async ValueTask<InnerUserPasswordDto> Handle(SetInstanceRootPasswordCommand command, CancellationToken cancellationToken)
        {
            ...
            if (!instance.IsManaged || instance.ContainerId is null)
                throw new InvalidOperationException("Root password can only be changed on CommandBlock-managed databases.");
