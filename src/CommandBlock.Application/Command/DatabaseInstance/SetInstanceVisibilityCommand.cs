...
namespace CommandBlock.Application.Command.DatabaseInstance
{
    ...
    /// orchestrator (docker compose etc.), not by CommandBlock.</para>
    /// </summary>
        ...
        : ICommandHandler<SetInstanceVisibilityCommand, DatabaseInstanceDto>
    {
        public async ValueTask<DatabaseInstanceDto> Handle(SetInstanceVisibilityCommand command, CancellationToken cancellationToken)
        {
            ...
            if (!instance.IsManaged || instance.ContainerName is null || instance.ContainerId is null)
                throw new InvalidOperationException("Visibility can only be changed on CommandBlock-managed databases. External containers are owned by the orchestrator that created them.");
