...
namespace CommandBlock.Application.Queries.Database
{
    ...
    /// Register flow. Skips:
    ///   - anything labelled commandblock.managed=true (CommandBlock provisioned it; already in the list)
    ...
    /// </summary>
    public class DiscoverContainersQueryHandler(CommandBlockDbContext db, IDockerService docker)
        : IQueryHandler<DiscoverContainersQuery, IReadOnlyList<DiscoveredContainerDto>>
    {
        public async ValueTask<IReadOnlyList<DiscoveredContainerDto>> Handle(DiscoverContainersQuery query, CancellationToken cancellationToken)
        {
            ...
            {
                // CommandBlock-managed containers are already in the instances list - don't re-offer.
                if (c.Labels is not null && c.Labels.TryGetValue("commandblock.managed", out var managed) && managed == "true")
                    continue;
                ...
                // Used downstream by the migration flow to (a) gate the "Migrate into CommandBlock"
        ...
        // Maps a Docker image name to one of CommandBlock's supported engine keys. Unknown images
