...
namespace CommandBlock.Application.Queries.App
{
    ...
    public class AppQueryHandler(IConfiguration configuration) : IQueryHandler<AppQuery, AppDto>
    {
        ...
        {
            // The login redirect is just the app's public URL, so derive it from CommandBlock:PublicUrl when
            // Oidc:RedirectUri isn't set explicitly - one less thing to configure (and to get wrong).
            var publicUrl = configuration["CommandBlock:PublicUrl"];
