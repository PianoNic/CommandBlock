...
// CommandBlock runs in one of two roles from the same image. "node" is a stripped worker that does nothing
...
if (string.Equals(builder.Configuration["CommandBlock:Role"], "node", StringComparison.OrdinalIgnoreCase))
{
    ...
    builder.Services.AddSingleton<CommandBlock.Infrastructure.Interfaces.INodeRpc, OfflineNodeRpc>();
    ...
    builder.Services.AddScoped<CommandBlock.Infrastructure.Interfaces.IDockerServiceResolver, LocalDockerServiceResolver>();
...
builder.Services.AddScoped<CommandBlock.Application.ConfigManagedGuard>();
...
builder.Services.AddScoped<CommandBlock.Infrastructure.Interfaces.IDockerServiceResolver, DockerServiceResolver>();
// Dispatches inner-DB operations to a node when the target carries a NodeId (used by the routing resolvers).
builder.Services.AddSingleton<CommandBlock.Infrastructure.Interfaces.INodeRpc, NodeRpc>();
