...
namespace CommandBlock.API.Controllers
{
        ...
        IActivityLogger activity) : ControllerBase
    {
        ...
        {
            // CommandBlock:PublicUrl is required for self-hosting (it also drives the OIDC redirect + CORS),
            // so it's always present here.
            var controlPlaneUrl = (configuration["CommandBlock:PublicUrl"] ?? options.Value.PublicUrl ?? "").TrimEnd('/');
        ...
        /// (they'd reappear on the next restart) - remove them from commandblock.yaml instead.</summary>
        [HttpDelete("{id:guid}")]
        ...
        public async Task<IActionResult> Delete(Guid id, CancellationToken cancellationToken)
        {
            ...
            if (node.IsConfigManaged) return Conflict("This node is managed by commandblock.yaml; remove it there.");
