...
namespace CommandBlock.API
{
    ...
    /// </list>
    /// Errors per entry are logged and skipped so a typo in one block doesn't take down CommandBlock.
    /// </summary>
        ...
        : BackgroundService
    {
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            {
                log.LogInformation("No instances file configured (commandblock.instances_file is unset). Skipping reconcile.");
                return;
            }
        ...
        private async Task ProvisionFromConfigAsync(ConfigInstance spec, CommandBlockDbContext db, IMediator mediator, CancellationToken cancellationToken)
        {
        ...
        private async Task ReconcileExistingAsync(
            CommandBlock.Domain.DatabaseInstance row,
            ConfigInstance spec,
