...
internal static class WizardHelper
{
    ...
    /// ContainerName is the actual Docker container name (e.g. commandblock-pg-xxxx), used for
    /// docker CLI assertions.
    /// </summary>
    public record ProvisionedInstance(string DisplayName, string ContainerName, string DefaultDb);

    public static async Task<ProvisionedInstance> ProvisionPostgresAsync(IPage page, CommandBlockStack stack, string defaultDbName)
    {
    ...
    public static async Task CleanupAsync(IPage page, CommandBlockStack stack, string displayName)
    {
        try
        {
