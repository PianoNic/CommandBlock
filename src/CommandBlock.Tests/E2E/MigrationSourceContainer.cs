...
/// `docker compose` would write. The guided-migration wizard surfaces a "Migrate into CommandBlock"
...
/// available wherever the e2e suite runs (CommandBlockStack uses it via FluentDocker).
/// </summary>
internal sealed class MigrationSourceContainer : IAsyncDisposable
{
    ...
    public static async Task<MigrationSourceContainer> StartPostgresAsync()
    {
        ...
        var name = $"commandblock-mig-src-{suffix}";
        ...
        var database = "src_db";
        var project = $"commandblock-mig-{suffix}";
        var service = "db";
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        // removed it (e.g. if CommandBlock adopted the container during the migration).
