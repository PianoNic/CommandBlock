...
public class MigrationTests
{
    ...
    ///   2. The "Migrate into CommandBlock" button gates correctly on compose-managed + supported
    ...
    ///
    /// We deliberately don't assert "Migration complete" here: the CommandBlock container running
    ...
    /// tracked as its own bug (CommandBlock-in-container -&gt; source-on-host requires either
    ...
    {
        var stack = await CommandBlockStack.GetAsync();
        await using var source = await MigrationSourceContainer.StartPostgresAsync();
        var session = await CommandBlockTestFixture.NewAuthenticatedSessionAsync(stack);
        ...
        try
        {
            ...
            var dialog = page.Locator("[role=dialog]").Filter(new() { HasText = "Migrate into CommandBlock" });
            await Assertions.Expect(dialog).ToBeVisibleAsync(new() { Timeout = 5000 });

            // Defaults: name = `<compose-service>-commandblock`, source password came from POSTGRES_PASSWORD,
            // source database from POSTGRES_DB. Just sanity-check the wizard pulled them through.
            await Assertions.Expect(dialog.Locator("#mig-name")).ToHaveValueAsync(source.ComposeService + "-commandblock");
