...
namespace CommandBlock.Tests.Services
{
    public class ConfigManagedGuardTests
    {
        ...
        private static CommandBlockDbContext NewDbContext()
        {
            var options = new DbContextOptionsBuilder<CommandBlockDbContext>()
                .UseInMemoryDatabase("guard-" + Guid.NewGuid())
                .Options;
            return new CommandBlockDbContext(options);
        }
        ...
        private static DatabaseInstance MakeInstance(bool isConfigManaged)
        {
            return new DatabaseInstance
            {
                ...
                Version = "18",
                DisplayName = "test",
                ContainerName = "commandblock-test",
