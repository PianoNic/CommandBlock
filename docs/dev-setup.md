...
- **Architecture quick map**:
  - `CommandBlock.Domain`: entities (`ServerInstance`, `BackupEntry`, `ActivityEntry`, `BaseEntity`), no dependencies
  - `CommandBlock.Application`: Mediator queries/commands + DTOs
  - `CommandBlock.Infrastructure`: EF DbContext + migrations + `Services/` (DockerService, S3BackupStorage, ServerStatusService, ActivityLogger) + `Interfaces/`
  - `CommandBlock.API`: ASP.NET Core entry point, JWT bearer auth, CORS, OpenAPI, controllers, the Minecraft router, DI wiring via `AddDocker()` / `AddActivityLog()` / etc.
